...
- **客户门户**：发起运输需求（pending → 转合同 / 驳回）、我的合同/账单、确认或异议对账；
- **司机端（H5）**：接单 → 扫码装货/卸货（装卸码校验）→ 发车/到达 → 电子签收（支持补签）→ 收入明细；
- **安全管理**：事故登记/关闭、安全培训（参训司机）、车辆安全检查、年检/驾照过期拦截派车；
- **仓储管理**：批次库存（FIFO 出库）、安全库存预警（跌破下限定向通知）、库存状态（normal/locked/临期）；
- **运价管理**：线路运价表（商品 × 装/卸场站 × 方式），合同新建可查表取价、调价留痕、启停；
- **系统管理**：用户（含数据范围设置）/ 角色（菜单 + 操作授权，数据化权限表）/ 操作日志（全量审计）。
...
| 层级 | 脚本 | 说明 |
|---|---|---|
| 服务层冒烟 | `npm test`（`scripts/verify-flow.mjs`） | 纯 Node：mock 数据一致性、状态机闭环、RBAC 默认拒绝、25 个业务环节逐条校验 |
...
历史审计与整改记录见 [docs/flow-audit.md](docs/flow-audit.md)（P0–P3 全部环节已整改闭环）。
