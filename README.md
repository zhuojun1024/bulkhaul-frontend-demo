...
npm run build        # 生产构建
npm test             # 服务层冒烟测试（verify-flow：数据一致性 + 状态机闭环 + RBAC + 25 个环节校验）
...
- **仓储管理**：批次库存（FIFO 出库）、手工入库/补库（采购到货，RBAC 守卫 + 容量校验）、安全库存预警（跌破下限定向通知）、库存状态（normal/locked/临期）；
...
| 层级 | 脚本 | 说明 |
|---|---|---|
| 服务层冒烟 | `npm test`（`scripts/verify-flow.mjs`） | 纯 Node：mock 数据一致性、状态机闭环、RBAC 默认拒绝、32 个业务环节逐条校验 |
...
历史审计与整改记录见 [docs/flow-audit.md](docs/flow-audit.md)（P0–P3 全部环节已整改闭环）；2026-08 二次审计修复（卸货中异常恢复/手工入库/对账明细/司机端异常上报/司机新增导入/重置密码/报表运量口径/拆车余数）见 [docs/fix-plan.md](docs/fix-plan.md)。
